...
class BertSelfAttention(nn.Module):
  ...
  def attention(self, key, query, value, attention_mask):
    ...
    ### TODO
    S = F.softmax(torch.div((query @ key.transpose(-1, -2)), key.shape[-1]**0.5) + attention_mask, dim=-1)
    S = (S @ value).transpose(1, 2)
    S = S.reshape(key.shape[0], key.shape[2], -1)
    return S
...
class BertLayer(nn.Module):
  ...
  def add_norm(self, input, output, dense_layer, dropout, ln_layer):
    ...
    return ln_layer(input + drop_output)
  ...
  def forward(self, hidden_states, attention_mask):
    ...
    ffn_output = self.interm_af(self.interm_dense(add_norm_output))
    output = self.add_norm(add_norm_output, ffn_output, self.out_dense, self.out_dropout, self.out_layer_norm)
    return output
...
class BertModel(BertPreTrainedModel):
  ...
  def embed(self, input_ids):
    ...
    return self.embed_dropout(self.embed_layer_norm(sum_embeds))
